...
const UI_STRINGS = {
  ...
  ar: {
    title: "اختبار الذكاء",
    desc: "تقييم تكيفي يعتمد على مستوى إتقانك.",
    loading: "جاري تجميع الأسئلة...",
    finished: "اكتمل التقييم",
    resultText: "معدل الدقة",
    tryOther: "تغيير الموضوع",
    scoreLabel: "النقاط",
    questionLabel: "سؤال",
    fromLabel: "من",
    explanationLabel: "التوليف الأكاديمي:",
    nextBtn: "التالي",
    finishBtn: "عرض النتيجة",
    questionsCount: "5 أسئلة",
    startBtn: "ابدأ الاختبار",
    levelUp: "ارتفع المستوى!",
    levelDown: "تم تعديل الصعوبة.",
    masteryLabel: "المستوى الحالي",
    levels: ["مبتدئ", "متوسط", "خبير"],
    multiSelectHint: "خيار معقد: اختر جميع الإجابات الصحيحة.",
    submitAnswer: "إرسال التحليل",
    topics: ['الوظيفية', 'نظرية الصراع', 'علم الاجتماع الرقمي', 'الديناميكيات الاجتماعية', 'الهوية والثقافة']
  }
};
            ...
            {q.options.map((opt, i) => {
              ...
              return (
                ...
                >
                  ...
                  {hasAnswered && isCorrect && <span className="bg-emerald-500 text-white w-8 h-8 rounded-full flex items-center justify-center">✓</span>}
                  {hasAnswered && isSelected && !isCorrect && <span className="bg-rose-500 text-white w-8 h-8 rounded-full flex items-center justify-center">✕</span>}
                </button>
              );
            })}
          </div>
          ...
          {hasAnswered && (
            <div className="bg-slate-900 text-white p-10 rounded-[2.5rem] border border-white/5 animate-in slide-in-from-top-4 duration-500 space-y-6">
              ...
              <button onClick={nextQuestion} className="w-full bg-indigo-600 text-white py-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-indigo-500 transition-all flex items-center justify-center gap-3">
                {currentIdx < questions.length - 1 ? t.nextBtn : t.finishBtn} ➔
              </button>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {t.topics.map((t_name) => (
          <button key={t_name} onClick={() => startQuiz(t_name)} className="group bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm hover:shadow-2xl transition-all text-center flex flex-col items-center relative overflow-hidden">
            ...
            <div className="w-24 h-24 bg-slate-50 rounded-[2rem] flex items-center justify-center text-5xl mb-8 group-hover:bg-emerald-600 group-hover:text-white transition-all transform group-hover:rotate-12">📝</div>
